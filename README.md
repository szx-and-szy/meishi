...
app.js
 ...
 ├── js/merchant.js       ← loadMerchants / selectMerchant / openMerchantUpload / handleMerchantUpload
 ├── js/review.js         ← writeReview / handleConfirmReview / reportReview / submitFeedback
 ├── js/admin.js          ← renderAdmin / approveMerchant / rejectMerchant / hideReview / dismissReports / 商家 CRUD
 ├── js/render.js         ← setActiveView / renderMerchants / renderDetail / renderProfile / renderReviewList / renderPhotoStrip
 ├── js/events.js         ← setupEventDelegation（data-action 事件委托 + 表单/导航/搜索监听）
 ├── js/image.js          ← compressImage（Canvas → WebP）/ uploadImageToStorage
 ├── js/scoring.js        ← bayesianScore / merchantSummary / getFilteredMerchants
...
| 表名 | 说明 |
|------|------|
| users | 用户（id, student_id, nickname, avatar_url, role, account_status, warning_count） |
...
| get_bayesian_score(uuid) | 计算商家贝叶斯评分 |
...
### v1.2.0
- 前端代码重构为 ES Modules 模块化架构（12 个模块）
- 新增启动画面（Splash Screen）
- 新增图片大图查看器
- 新增青春市集 SVG 地图（支持双指缩放）
- 新增头像上传功能（自动压缩为 WebP）
- 新增个人资料编辑（修改昵称、修改密码）
- 新增忘记密码对话框
- 新增 Toast 通知系统
- 新增全局 Loading 遮罩
- 新增 safeApiCall 统一 API 错误处理
- 新增搜索防抖（300ms）
- 管理员工作台新增商家名称/位置内联编辑
- 管理员工作台新增封面图修改
- 管理员工作台新增照片删除功能
